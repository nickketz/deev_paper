...
%DIF DEL manuscript_old.tex   Sun May 19 21:22:55 2019
%DIF ADD ../manuscript.tex    Sun May 19 21:18:34 2019
...
Through the use of neural network models of the hippocampus and surrounding cortex this work proposes a framework for understanding how time frequency signatures measured at the scalp can be used to track declarative memory processes, and make quantitative predictions about how information \DIFdelbegin \DIFdel{in }\DIFdelend stored in long term memory is altered by these processes.  The fundamental hypothesis explored here is that neural oscillations in the theta (3-8 Hz), alpha (8-12 Hz) frequency bands can be tied to specific neural mechanisms supporting declaritive memory, and that these oscillatory signatures can be tracked in human scalp EEG recordings to predict behavioral changes in declarative memory processes. Specifically\DIFdelbegin \DIFdel{that }\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend oscillatory power in the theta band positively correlates with the how much information the hippocampus is reactivating for a given retrieval event, and that power in the alpha band positively correlates with how much information is being inhibited from being retrieved.  We first explore this idea through computational simulations of thalamo-cortical-hippocampal interactions which make a series of predictions tested in a human EEG experiment using the Dependent Events paradigm.  Results largely support the idea that thalamically mediated changes in alpha power during episodic memory encoding and retrieval are related to the flow of information between the neocortex and hippocampus, and that the bandwidth of this information flow tracks with theta power.  Further, this model suggest the encoding \DIFdelbegin \DIFdel{a }\DIFdelend \DIFaddbegin \DIFadd{and }\DIFaddend retrieval processes are inter-dependent and that attentional modulation of what gets retrieved from long term memory subsequently impacts the \DIFdelbegin \DIFdel{underlying representation that information }\DIFdelend \DIFaddbegin \DIFadd{information that }\DIFaddend is encoded into \DIFaddbegin \DIFadd{the underlying representation}\DIFaddend .
...
}\DIFdelend %DIF > An example of what this looks like in the network is shown in Figure \ref{fig:netNonTarg}B, with more pattern completion occurring in the location stimulus dimension (both in ECout and the TRC layer) for the closed-loop condition compared to open-loop.
...
\begin{figure}
    \begin{center}
	\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\textwidth]{figs/depNonTarg.pdf}
%DIFDELCMD <     %%%
...
\begin{figure}
	\begin{center}
		\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\textwidth]{figs/deevEEGBeh.pdf}
%DIFDELCMD < 	%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\textwidth]{figs/deevEEGBeh_edit.pdf}
	\DIFaddendFL \end{center}
	\caption[EEG Dependent Events Behavior]{
		...
		\emph{(B)} \DIFdelbeginFL \DIFdelFL{Relationship between retrieval confidence and accuracy.  Scatter plot of average cued-recognition accuracy within }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Average Dependency over }\DIFaddendFL subjects \DIFdelbeginFL \DIFdelFL{as a function of self rated confidence, broken down by open and closed-loop conditions.  Overlayed histograms show smoothed marginal distributions }\DIFdelendFL for \DIFdelbeginFL \DIFdelFL{open }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{both open-loop }\DIFaddendFL and closed-loop conditions.  \DIFdelbeginFL \emph{\DIFdelFL{(C)}} %DIFAUXCMD
\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Blue bars show }\DIFaddendFL Dependency \DIFdelbeginFL \DIFdelFL{difference as a function of block. Dependency }\DIFdelendFL assuming underlying event structure referred to as `data', and \DIFaddbeginFL \DIFaddFL{red bars are }\DIFaddendFL assuming all cue-targ trial types are independent referred to as `\DIFdelbeginFL \DIFdelFL{Independent}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{indp}\DIFaddendFL '. \DIFaddbeginFL \DIFaddFL{Error bars show 95\% confidence intervals. 
		}\emph{\DIFaddFL{(C)}} \DIFaddFL{Dependency difference as a function of block.   }\DIFaddendFL Dash lines show mean across blocks, and error bars show 95\% confidence intervals.
	}
...
\DIFaddbegin \DIFadd{In general this work provides another perspective on the same mechanisms of the Sync/deSync model \mbox{%DIFAUXCMD
\cite{ParishHanslmayrBowman18}}%DIFAUXCMD
.  Here, we propose a specific thalamic mechanism responsible for the alpha dynamics witnessed in declarative memory processes and implement it in a rate coding neural network model.  This model, however, does not directly manifest alpha dynamics that can be directly measured, it only indirectly models them through the thalamo-coritical-hippocampal interactions.  \textcite{ParishHanslmayrBowman18}, use a spiking model to ellicit measures that are more directly related to the oscillatory dynamics that both their and our models are interested in.  Similarly, varying levels of detailed thalamo-cortical models provide solid ground for exploring the attention mechanisms proposed here \mbox{%DIFAUXCMD
\cite{HindriksvanPutten13, BeckerKnockRitterEtAl15}}%DIFAUXCMD
.   Future work can try and bridge this gap by including the spiking dynamics of the Sync/deSync model with the systems level mechanisms of our model.
}

\DIFaddend \subsection{Behavioral and EEG Support}
...
\DIFaddbegin \DIFadd{These results provide another data point in the Dependent Events paradigm, and helps illustrate the temporal dynamics that was not previously possible in the fMRI studies.  Specifically, the response locked theta effects and stimulus locked alpha effects both support the mechanism proposed by the computational model, and provide extra insight into the dynamics support the Dependent Events paradigm.    
}
